(function () {
    ...
    stockModule.controller('GrupoController', GrupoController);

    GrupoController.$inject = ['$scope', '$rootScope', '$location', 'RESTfulResourceService', 'notificationsFactory'];
    function GrupoController($scope, $rootScope, $location, RESTfulResourceService, notificationsFactory) {
        
    	// Verificando permissão de acesso ao recurso.
    	$rootScope.$watch('AUTORIZADOR.recursos', function() {
    		if (!$rootScope.AUTORIZADOR.isRecursoPermitido($rootScope.AUTORIZADOR.REC_CAD_GRUPO_TRANSACAO)) {
    	...
    	function init() {
    		$rootScope.AUTORIZADOR.carregarAcoesPermitidas($rootScope.AUTORIZADOR.REC_CAD_GRUPO_TRANSACAO);
    		$scope.buscar();
    	}
        ...
        function buscar() {
            ...
            RESTfulResourceService.buscar($rootScope.ENTITIES.GRUPO, $scope.pageSize, $scope.currentPage, $scope.filtro, function (response) {
                if (response.success) {
        ...
        function incluir() {
        	$scope.dataLoading = true;
        	RESTfulResourceService.incluir($rootScope.ENTITIES.GRUPO, $scope.item, function (response) {
                if (response.success) {
                	$scope.buscar();
                	$scope.dataLoading = false;
        ...
        function alterar() {
        	$scope.dataLoading = true;
        	RESTfulResourceService.alterar($rootScope.ENTITIES.GRUPO, $scope.item, function (response) {
                if (response.success) {
                	$scope.buscar();
                	$scope.dataLoading = false;
                	notificationsFactory.success();
        ...
        function excluir() {
        	$scope.dataLoading = true;
        	RESTfulResourceService.excluir($rootScope.ENTITIES.GRUPO, $scope.item, function (response) {
                if (response.success) {
                	$scope.buscar();                	
                	$scope.dataLoading = false;                	
