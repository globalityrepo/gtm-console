(function () {
    ...
    stockModule.controller('EventoTipoController', EventoTipoController);

    EventoTipoController.$inject = ['$scope', '$rootScope', '$location', 'RESTfulResourceService', 'notificationsFactory'];
    function EventoTipoController($scope, $rootScope, $location, RESTfulResourceService, notificationsFactory) {
        
    	// Verificando permissão de acesso ao recurso.
    	$rootScope.$watch('AUTORIZADOR.recursos', function() {
    		if (!$rootScope.AUTORIZADOR.isRecursoPermitido($rootScope.AUTORIZADOR.REC_CAD_TIPO_EVENTO)) {
    	...
    	function init() {
    		$rootScope.AUTORIZADOR.carregarAcoesPermitidas($rootScope.AUTORIZADOR.REC_CAD_TIPO_EVENTO);
    		$scope.buscar();
        ...
        function buscar() {
            ...
            RESTfulResourceService.buscar($rootScope.ENTITIES.EVENTO_TIPO, $scope.pageSize, $scope.currentPage, $scope.filtro, function (response) {
                if (response.success) {
        ...
        function incluir() {
        	$scope.dataLoading = true;
        	RESTfulResourceService.incluir($rootScope.ENTITIES.EVENTO_TIPO, $scope.item, function (response) {
                if (response.success) {
                	$scope.buscar();
                	$scope.dataLoading = false;
        ...
        function alterar() {
        	$scope.dataLoading = true;
        	RESTfulResourceService.alterar($rootScope.ENTITIES.EVENTO_TIPO, $scope.item, function (response) {
                if (response.success) {
                	$scope.buscar();
                	$scope.dataLoading = false;
                	notificationsFactory.success();
        ...
        function excluir() {
        	$scope.dataLoading = true;
        	RESTfulResourceService.excluir($rootScope.ENTITIES.EVENTO_TIPO, $scope.item, function (response) {
                if (response.success) {
                	$scope.buscar();                	
                	$scope.dataLoading = false;                	
